...
def get_model():
    return model(num_classes=num_classes, img_height=img_height, img_width=img_width, img_channels=img_channels)

model = get_model()
model.compile(optimizer="sgd", loss="categorical_crossentropy", metrics=["accuracy"])
model.summary()

history = model.fit(x_train, y_train_cat, batch_size=16, verbose=1, epochs= 100, validation_data=(x_test, y_test_cat), shuffle=False)#, class_weight=class_weights )
model.save("seg100accsgd.hdf5")
